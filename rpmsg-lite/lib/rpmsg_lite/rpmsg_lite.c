...
int rpmsg_lite_is_link_up(struct rpmsg_lite_instance *rpmsg_lite_dev)
{
    ...
    k_sleep(K_MSEC(1000));
//PPP    printk("Nunca cai aqui 0: %d\n",rpmsg_lite_dev);
//ppp    printk("Nunca cai aqui 1: %d\n",rpmsg_lite_dev->link_state);

//    if (rpmsg_lite_dev == 0) 
    // EU QUE MUDEI ISSO AKI
    if (!rpmsg_lite_dev)
  ...
  ///ppp	printk("Nunca cai aqui 2: %d\n",rpmsg_lite_dev);

  ///ppp  printk("Nunca cai aqui 1.1: %d\n",rpmsg_lite_dev->link_state);
...
//int
uint32_t rpmsg_lite_release_rx_buffer(struct rpmsg_lite_instance *rpmsg_lite_dev, void *rxbuf)
{
    struct rpmsg_std_msg *rpmsg_msg;
    struct rpmsg_hdr_reserved *reserved = RL_NULL;
